...
#///// ADD NEW WORK VERSION //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
""" - BÜYÜK KISMIN GEÇİCİ OLARAK DEVRE DIŞI OLMASI


#//////////////////////NEW FUNCTION - view -begin
@app.route('/viewworks/', methods=["GET", "POST"])
@login_required
def view_works(): #Add user - ALSO TO DATABASE!!!
    pending_records = None
    print("type", type(active_user_id))
    print("From viewworks -- ////////////",active_user_id)
    active_user = Staff_db.query.filter_by(id=active_user_id).first()
    user_role = active_user.role_db
    user_name = active_user.name_db
    
    if user_role=="user1":
        pending_records = Works_db.query.filter_by(user1_evaluation_db="Pending")
        all_records = Works_db.query.order_by(Works_db.id).all()
        return  render_template("viewworks.html", all_records=all_records, pending_records=pending_records, user_name=user_name)
    elif user_role=="user2":
        pending_records = Works_db.query.filter_by(user2_evaluation_db="Pending", user1_evaluation_db = "Accept") #user2_evaluation_db="Pending", user3_evaluation_db="Accept"
        all_records = Works_db.query.order_by(Works_db.id).all()
        return  render_template("viewworks.html", all_records=all_records, pending_records=pending_records, user_name=user_name)
    elif user_role=="user3":
        pending_records = Works_db.query.filter_by(user3_evaluation_db="Pending", user1_evaluation_db = "Accept", user2_evaluation_db = "Accept")
        all_records = Works_db.query.order_by(Works_db.id).all()
        return  render_template("viewworks.html", all_records=all_records, pending_records=pending_records, user_name=user_name)
    #User.query.order_by(User.popularity.desc(), User.date_created.desc()).limit(10).all()
    #all_records = Works_db.query.order_by(Works_db.id).all()
    #return  render_template("viewworks.html", all_records=all_records, pending_records=pending_records)

#//////////////////////NEW FUNCTION - view -end

#//////////////////////NEW FUNCTION * technıcal_ entry - begin

@app.route('/tech_office_entry/<int:id>', methods=["GET", "POST"])
@login_required
def tech_office_entry(): #Adding technical information to work by technical office team
    
    form = Technical_entry()
    #active_user_id # Aşağıda kullanıyoruz.. doğru mu?
    
    #Pending works.. satıştan gelen onaylanmış (accept) işler ile teknik ofiste Pending olarak bekleyenleri filtreleyeceğiz..
    pending_works = Works_db.query.get_or_404(id)
    pending_works = Works_db.query.filter_by(user1_evaluation_db="Accept", user2_evaluation_db="Pending")

    #Role kontrolü ve authorz...??? - 10.12.21
    test_user = Staff_db.query.filter_by(id=active_user_id).first()
    user_role = test_user.role_db
    user_name = test_user.name_db
    pending_works = #Tanımlanacak..
    
    #Validation of our form
    if form.validate_on_submit():
        
        tech_information = Works_db(

        order_no_db =form.order_no.data,
        circumference_db =form.circumference.data,
        area_db =form.area.data,
        weigth_in_gr_db =form.weigth_in_gr.data,
        press_db =form.press.data,
        difficulty_db =form.difficulty.data
        )
        
        db.session.add(tech_information)
        db.session.commit()
        flash("Teknik ofis bilgileri başarılı biçimde kaydedildi! Teşekkürler!", "success")
        ...
        flash("Teknik ofis bilgilerinin kaydında bir sorun oluştu. Lütfen IT Yöneticisi ile temasa geçiniz.", 'error')
        
    #User.query.order_by(User.popularity.desc(), User.date_created.desc()).limit(10).all()
    all_works = Works_db.query.order_by(Works_db.date_added_db).all()
    return  render_template("tech_office_entry.html", form=form, all_works = all_works)




#//////////////////////NEW FUNCTION * technıcal_ entry - end
...
@app.route('/viewworks/', methods=["GET", "POST"])
@login_required
def viewworks(): #Add user - ALSO TO DATABASE!!!
    pending_records = None
    print("type", type(active_user_id))
    print("From viewworks -- ////////////",active_user_id)
    test_user = Staff_db.query.filter_by(id=active_user_id).first()
...
@app.route('/viewworks2//<int:id>', methods=["GET", "POST"])
@login_required
def viewworks2(id): #Add user - ALSO TO DATABASE!!!
    form = Works_result()
    work_to_show = Works_db.query.get_or_404(id)
    test_user = Staff_db.query.filter_by(id=active_user_id).first()
    user_role = test_user.role_db
    ...
    if request.method == "POST":
        if user_role=="user1":
            work_to_show.user1_evaluation_db=request.form["evaluation"]        
        if user_role=="user2":
            work_to_show.user2_evaluation_db=request.form["evaluation"]
        if user_role=="user3":    
            work_to_show.user3_evaluation_db=request.form["evaluation"]
        try:
            db.session.commit()
            flash("Work information updated successfully.", "success")
            return render_template("viewworks2.html", form=form, work_to_show=work_to_show, id=id)
            
        except:
            db.session.commit()
            flash("Update process failed! Please inform your administrator..", "success")
            return render_template("index.html", form=form, work_to_show=work_to_show, id=id)
    
    
    return  render_template("viewworks2.html", work_to_show=work_to_show, id=id, form=form, user_name=user_name)
    
    
#////////////////////////////////

#This route is for changing all status of all records to PENDING.
@app.route('/modify_pending/', methods=["GET", "POST"])
@login_required
def modify_pending(): 
    all_records = Works_db.query.order_by(Works_db.id).all()
    for record in all_records:
        print(record.id, record.user1_evaluation_db, record.user2_evaluation_db, record.user3_evaluation_db)
        record.user1_evaluation_db = "Pending"
        record.user2_evaluation_db = "Pending"
        record.user3_evaluation_db = "Pending"
    try:
            db.session.commit()
            flash("Work information updated successfully.", "success")
            return render_template("viewworks2.html")
...
@app.route('/show_all_phases/', methods=["GET", "POST"])
@login_required
def show_all_phases(): #Add user - ALSO TO DATABASE!!!
    all_records = Works_db.query.order_by(Works_db.id).all()
    for record in all_records:
        print(record.id, record.user1_evaluation_db, record.user2_evaluation_db, record.user3_evaluation_db)
    return render_template("index.html")
